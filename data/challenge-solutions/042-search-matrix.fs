...
: flat@ ( i -- n )
  ch-cols-used /mod ch-grid@ ;

: matrix-search? ( key -- flag )
  { key }
  ch-cols-used ch-rows-used * { hi }
  0 { lo }
  begin  hi lo -  while
    lo hi + 2/ { mid }
    mid flat@ key < if  mid 1+ to lo  else  mid to hi  then
  repeat
  lo ch-cols-used ch-rows-used * u< if
    lo flat@ key = if  true  else  false  then
  else
    false
  then ;
