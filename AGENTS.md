...
**Code / challenges** — stack, style, control, factoring (`forth-*.mdc`); ignore architecture docs unless the task is porting or target-specific. Train solve queue **complete** (98/98); use `eval_holdout` for model validation (`docs/CHALLENGE-TO-TRAIN.md`). If `T{ }T` reports `WRONG NUMBER OF RESULTS`, check stack depth after the word (`docs/AGENT-SOLVE-CHALLENGES.md` §5b, `forth-control.mdc`).

**External LLM (Opus) + toolchain** — powerful cloud models are **orchestrators**, not postfix compilers; IR → transpiler → gforth. Hub: [`docs/AI-KNOWLEDGE-INDEX.md`](docs/AI-KNOWLEDGE-INDEX.md). Practical checklist: [`docs/GFORTH-AI-ECOSYSTEM.md`](docs/GFORTH-AI-ECOSYSTEM.md). Cursor skills (29): [`docs/GFORTH-SKILLS-CATALOG.md`](docs/GFORTH-SKILLS-CATALOG.md), install via `./install.sh`. See also [`docs/EXTERNAL-LLM-ARCHITECTURE.md`](docs/EXTERNAL-LLM-ARCHITECTURE.md), [`docs/AI-VS-TOOLS.md`](docs/AI-VS-TOOLS.md), [`docs/ROADMAP-AI-PLATFORM.md`](docs/ROADMAP-AI-PLATFORM.md).
