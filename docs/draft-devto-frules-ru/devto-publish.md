...
> **Повторы знаний:** метки `[*1]`…`[*12]` — см. [`REPETITIONS-MAP.md`](REPETITIONS-MAP.md). «КАНON» = оставить в dev.to; «ПОВТОР/СЫРОЙ ЛОГ» = срезать.
...
[*12] Я вдруг подумал, что ИИ просто не знает, что такое Forth и надо ему просто дать всю информацию. Подумал я так, конечно же, не на ровном месте, а после длительных диалогов в чате с ИИ. После чего мы пришли к тому, что нужно собрать датасет со знаниями, и уж тогда модель сможет программировать!
...
В процессе решения задач, ИИ выдает очень много терминов из области ИИ. Я их собрал и начал обсуждения. Кроме этого, проведя эксперименты и замеры - нужно ведь правильно трактовать результат, уметь делать выводы, а я их делать не умею. Поэтому у нас начался длительный этап общения с ИИ с целью понять, а что же мы сделали, какой сделать вывод и что делать дальше. И вот мы с ней общались и общались, у меня появлялись новые вопросы, появлялись новые термины. Я по ним задавал дополнительные вопросы. Это хороший механизм узнать то, что ты сам не знаешь (мы не можем знать то, чего не знаем). Постепенно раскрутили диалог.

Некоторые вопросы я задавал неоднократно, несколько раз возвращаясь к ним. Видимо, мне просто не хотелось верить в некоторые свои заблуждения.

Вообще, мне ИИ с самого начала сказала, что я не прав, что мне модель 0.5В не подходит, и что мне вообще не надо генерировать код на Forth. Но я не хотел с этим соглашаться. И для того, чтобы к этому прийти - у меня произошло несколько стадий принятия )))
...
Изначально взяв маленькую модель для проверки пайплайна - я к ней, видимо, привязался и хотел от нее получить какую-то реальную пользу. В очередной раз обсудив с ИИ размер сети, я в очередной раз получил ответ, что от малой модели рабочих решений на Forth ждать не стоит.
...
И вот тут я начинаю догадываться, что иду вообще не туда. Я то хочу получить от неронки прямую генерацию кода на Forth! Но требовать от сети генерацию обратной польской нотации (RPN (1 2 +)) - напрямую это ошибка. Она ни в силу малого объема обучающих данных, ни в силу своей природы - последовательно генерировать слова - вообще не может отменить свои слова и переставить их (без применения внешнего инструмента).
...
После этого я сдался. И признал, что надо делать именно так. Тем более, я уже начал догадываться, что прямая работа с синтаксисом и нотацией - это еще не программирование. Нужно ведь еще алгоритмы понимать, структуры данных и иметь опыт к решению задач используя алгоритмы и структуры. Надо пройти от простого к сложному (`curriculum`), а я ведь не объяснил сети даже простых вещей, и сразу начал требовать с нее решения сложных задач. А нужно быть обученным на всех уровнях (наверное). На решение алгоритмических задач! И Coder содели как раз на это и обучены и надо использовать результат их работы. И этот результат уже преобразовывать в Forth не нейронным способом.
...
Далее я прихожу к выводу о том, что итоговый Forth результат можно получить вообще другим путем - через внешний инструмент преобразования. В составе набора инструментов вокрук ИИ модели - так называемого "завода".
...
Чему модель не может научиться (или не стоит ждать) от ИИ (архитектурные ограничения):
...
Таким образом, мы пришли к пониманию и изобрели такое понятие, которое все называют "навыки".

Осознание: монолитных ИИ нейронок, которые умеют программировать - нет. мы ими не пользуемся. мы  ИИ экосистемами, которые умеют в итоге получить программный код. а дома мы запускаем одну моно модель. и потом удивляемся - я че она такая тупая. Но мы сравниваем одну маленькую модель с большим наобором больших моделей и инструментов вокруг них.
...
1. «Пусть Opus напишет весь `: word ... ;` — overhard + overkill + overinvoice (F3).
2. «LoRA научит postfix» — нет.
3. Ждать от 0.5B работоспособности - нет.
4. Писать код на Forth, там где в него можно преобразовать

Скрин с оплатами за курсор вставить 


**Одна модель без инструментов - не программирует.** Программирует **система**: LLM(и) + transpiler + gforth + flint/fcov + человек. 

[*7] ## Завод вместо монолита — КАНON; не дублировать § «Локальный завод» ~996
